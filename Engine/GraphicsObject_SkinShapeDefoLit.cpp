...
GraphicsObject_SkinShapeDefoLit::GraphicsObject_SkinShapeDefoLit(Model *model, ShaderObject *_pShaderObj, TextureName name, Vect &LightColor, Vect&LightPos, bool transparent)
	...
	//ShapeWeightsArray(_pShaderObj->GetLocation("shapeWeights"))
{
	...
	glUniformBlockBinding(pShaderObj->getProgramObject(), _pShaderObj->GetBufferLocation("light"), 1);

}

void GraphicsObject_SkinShapeDefoLit::SetState()
{
...
void GraphicsObject_SkinShapeDefoLit::SetDataGPU()
{
	// use this shader
...
void GraphicsObject_SkinShapeDefoLit::Draw()
{
	// draw
...
void GraphicsObject_SkinShapeDefoLit::RestoreState()
{
	glDisable(GL_BLEND);
	glEnable(GL_CULL_FACE);
	glFrontFace(GL_CW);
